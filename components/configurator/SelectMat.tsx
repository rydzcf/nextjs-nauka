...
export default function SelectMat({
  req,
  setMatSpring,
  setMatH,
  setMatBuild,
  ...
  const uniqueH = useMemo(() => {
    ...
    return Array.from(new Set(hs as string[])).sort((a: string, b: string) =>
      a.localeCompare(b)
    );
  }, [data, req.matSpring]);
  ...
  const uniqueZone = useMemo(() => {
    ...
    const zones = data
      ...
      .filter((value) => value !== null && value !== undefined);
    return Array.from(new Set(zones as string[])).sort((a: string, b: string) =>
      a.localeCompare(b)
    );
  }, [data, req.matSpring]);
  ...
  const uniqueBuild = useMemo(() => {
    ...
    const builds = data
      .filter(
        (product) => product.spring === req.matSpring && product.h === req.matH
      )
      .map((product) => product.build)
      .filter((value) => value !== null && value !== undefined);
    return Array.from(new Set(builds as string[])).sort(
  ...
  return (
    <>
      <H1>Wybierz sprężynę materaca<Mark value={req.matSpring ?? null}></Mark></H1>
      ...
      {req.matSpring ? (
        <>
          <H1>Wybierz polowość<Mark value={req.matZone ?? null}></Mark></H1>
          <div className="flex flex-wrap justify-center">
            {uniqueZone.map((zone) => {
              const product = data.find(
                (product) =>
                  product.zone === zone && product.spring === req.matSpring
              );
              return (
                <div key={zone + ""}>
                  {product && (
                    <Option
                      ...
                      visibleName={zone ?? ""}
                      handleSelected={() =>
                        setMatZone((zone as Req["matZone"]) ?? null)
                      }
      ...
      {req.matSpring === "TFK" && req.matZone ? (
        <>
          <H1>Wybierz twardość sprężyny<Mark value={req.matH ?? null}></Mark></H1>
          <div className="flex flex-wrap justify-center">
            {uniqueH.map((h) => {
              const product = data.find(
                (product) => product.h === h && product.spring === req.matSpring
              );
              return (
                <div key={h + ""}>
                  {product && (
      ...
      {req.matSpring && req.matZone ? (
        <>
          <H1>Wybierz budowę<Mark value={req.matBuild ?? null}></Mark></H1>
          <div className="flex flex-wrap justify-center">
            {uniqueBuild.map((build) => {
              const product = data.find(
                (product) =>
                  product.h === req.matH &&
                ...
                <div key={build + ""}>
                  {product && (
                    <Option
                      ...
                      handleSelected={() =>
                        setMatBuild(
                          (JSON.stringify(product) as Req["matBuild"]) ?? null
                        )
                      }
                      {...(req.matBuild === JSON.stringify(product) && {
                        active: true,
                      })}
                    />
                  )}
